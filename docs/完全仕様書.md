# Slide Voice Maker 完全仕様書

**バージョン**: 1.0.0
**日付**: 2026-01-05
**リポジトリ**: https://github.com/J1921604/Slide-Voice-Maker

---

## 目次

1. [概要](#概要)
2. [システムアーキテクチャ](#システムアーキテクチャ)
3. [機能仕様](#機能仕様)
4. [データモデル](#データモデル)
5. [CLI仕様](#cli仕様)
6. [Web版仕様](#web版仕様)
7. [ファイル構造](#ファイル構造)
8. [デプロイ](#デプロイ)
9. [テスト](#テスト)

---

## 概要

Slide Voice MakerはPDFスライドと原稿CSV（`index,script`形式）からAI音声ナレーション付き動画（WebM）を自動生成するツールです。

### 主要機能

| 機能 | 説明 | 優先度 |
|------|------|--------|
| 解像度選択 | 720p/1080p/1440pから選択可能 | P1 |
| temp上書き更新 | 毎回tempフォルダを自動クリア | P1 |
| Web版対応 | GitHub Pagesでブラウザ実行可能 | P2 |
| CSV読み込み | UTF-8/Shift_JIS/EUC-JP対応 | P1 |
| 音声合成 | Edge TTS（ローカル版）/ GitHub Actions（Web版） | P1 |

### ローカル版機能（src/server.py起動時）

| 機能 | 説明 |
|------|------|
| PDF入力 | inputフォルダにPDFファイルを上書き保存 |
| 原稿CSV入力 | inputフォルダにCSVファイルを上書き保存 |
| 解像度選択 | 720p/1080p/1440pプルダウン |
| 音声生成 | Edge TTSで実際のAI音声を生成 |
| 動画出力 | output/tempに上書き更新、output/*.webm生成 |
| ダウンロード | outputフォルダから選択したwebmをダウンロード |

### Web版機能（GitHub Pages）

| 機能 | 説明 |
|------|------|
| 静的ホスティング | 外部APIキー不要で設計 |
| 音声生成 | GitHub Actions（generate-video.yml）でEdge TTS実行 |
| 入力方法 | リポジトリのinput/にPDF/CSVを配置してActionsを手動実行 |

### 動作環境

| 項目 | 要件 |
|------|------|
| OS | Windows 10/11 |
| Python | 3.10.11（推奨） |
| ブラウザ | Chrome / Edge（最新版） |

---

## システムアーキテクチャ

### 全体構成図

```mermaid
flowchart TB
    subgraph ユーザー入力
        PDF[PDFファイル]
        CSV[原稿CSV]
        RES[解像度選択]
    end

    subgraph Python版
        MAIN[src/main.py]
        PROC[src/processor.py]
        TTS[Edge TTS]
        FFMPEG[FFmpeg]
    end

    subgraph Web版
        HTML[index.html]
        PDFJS[PDF.js]
        WSAPI[Web Speech API]
    end

    subgraph 出力
        TEMP[output/temp/]
        WEBM[output/*.webm]
    end

    PDF --> MAIN
    CSV --> MAIN
    RES --> MAIN
    MAIN --> PROC
    PROC --> TTS
    PROC --> FFMPEG
    FFMPEG --> TEMP
    FFMPEG --> WEBM

    PDF --> HTML
    CSV --> HTML
    RES --> HTML
    HTML --> PDFJS
    HTML --> WSAPI
    HTML --> WEBM
```

### データフロー

```mermaid
sequenceDiagram
    participant User as ユーザー
    participant Main as main.py
    participant Proc as processor.py
    participant TTS as Edge TTS
    participant FFmpeg as FFmpeg

    User->>Main: --resolution 1080p指定
    Main->>Main: RESOLUTION_MAP参照
    Main->>Main: OUTPUT_MAX_WIDTH=1920設定
    Main->>Proc: process_pdf_and_script()
    Proc->>Proc: clear_temp_folder()
    Proc->>Proc: PDF→画像変換
    Proc->>TTS: 音声生成リクエスト
    TTS-->>Proc: 音声ファイル
    Proc->>Proc: _get_output_max_width()→1920
    Proc->>FFmpeg: エンコード（1920x1080）
    FFmpeg-->>Proc: WebMファイル
    Proc-->>Main: 完了
    Main-->>User: 動画出力完了
```

### 状態遷移

```mermaid
stateDiagram-v2
    [*] --> 待機中: アプリ起動

    待機中 --> 入力確認中: PDF/CSV選択
    入力確認中 --> エラー: 入力不正
    入力確認中 --> 解像度選択中: 入力OK

    解像度選択中 --> 処理中: 生成開始
    処理中 --> temp削除中: clear_temp_folder
    temp削除中 --> PDF変換中: 削除完了
    PDF変換中 --> 音声生成中: 変換完了
    音声生成中 --> エンコード中: 音声完了
    エンコード中 --> 完了: エンコード完了
    エンコード中 --> エラー: エンコード失敗

    エラー --> 待機中: リトライ
    完了 --> 待機中: 新規生成
    完了 --> [*]: 終了
```

---

## 機能仕様

### FR-001: 解像度選択

| 選択肢 | 幅（px） | 高さ（px） | 環境変数値 |
|--------|----------|------------|------------|
| 720p   | 1280     | 720        | 1280       |
| 1080p  | 1920     | 1080       | 1920       |
| 1440p  | 2560     | 1440       | 2560       |

**実装**:

```python
RESOLUTION_MAP = {
    "720": 1280,
    "720p": 1280,
    "1080": 1920,
    "1080p": 1920,
    "1440": 2560,
    "1440p": 2560,
}
```

### FR-002: temp上書き機能

```python
def clear_temp_folder(temp_dir: str) -> None:
    """tempフォルダを上書きクリア"""
    if os.path.exists(temp_dir):
        try:
            shutil.rmtree(temp_dir)
            print(f"Cleared temp folder: {temp_dir}")
        except PermissionError as e:
            print(f"Warning: Could not clear temp folder: {e}")
    os.makedirs(temp_dir, exist_ok=True)
```

### FR-003: CSV読み込み（文字化け対処）

対応エンコーディング:
1. UTF-8
2. Shift_JIS
3. EUC-JP
4. ISO-2022-JP

**CSV形式**:

```csv
index,script
0,"最初のスライドの原稿テキスト"
1,"2番目のスライドの原稿（複数行可）"
```

---

## データモデル

### エンティティ関係図

```mermaid
erDiagram
    Project ||--o{ PDFDocument : "入力として持つ"
    Project ||--o| ScriptCSV : "原稿として持つ"
    Project ||--|| Resolution : "出力解像度を持つ"
    Project ||--|| TempFolder : "一時フォルダを持つ"
    
    PDFDocument ||--|{ Slide : "ページを含む"
    ScriptCSV ||--|{ ScriptEntry : "原稿を含む"
    
    Slide ||--o| ScriptEntry : "原稿と対応"
    Slide ||--o| AudioFile : "音声を持つ"
    Slide ||--o| ImageFile : "画像を持つ"
    
    VideoOutput ||--|{ Slide : "スライドを含む"
    VideoOutput ||--|| Resolution : "解像度を持つ"
    
    TempFolder ||--o{ AudioFile : "格納する"
    TempFolder ||--o{ ImageFile : "格納する"
```

### エンティティ定義

#### Resolution

| 属性 | 型 | 説明 |
|------|-----|------|
| value | string | "720p", "1080p", "1440p" |
| width | int | 1280, 1920, 2560 |
| height | int | 720, 1080, 1440 |

#### Slide

| 属性 | 型 | 説明 |
|------|-----|------|
| page_index | int | ページ番号（0始まり） |
| image_path | string | 画像ファイルパス |
| audio_path | string | 音声ファイルパス |
| script_text | string | 原稿テキスト |
| duration | float | 表示時間（秒） |

---

## CLI仕様

### 基本構文

```bash
py -3.10 src/main.py [オプション]
```

### 引数定義

| 引数 | 型 | デフォルト | 説明 |
|------|-----|-----------|------|
| `--input` | string | `input/` | 入力ディレクトリパス |
| `--output` | string | `output/` | 出力ディレクトリパス |
| `--script` | string | `input/原稿.csv` | 原稿CSVファイルパス |
| `--resolution` | string | `720` | 出力解像度 |

### 使用例

```bash
# デフォルト設定
py -3.10 src/main.py

# 1080p出力
py -3.10 src/main.py --resolution 1080p

# フルオプション
py -3.10 src/main.py --input ./input --output ./output --script ./input/原稿.csv --resolution 1440p
```

### 終了コード

| コード | 意味 |
|--------|------|
| 0 | 正常終了 |
| 1 | 入力ファイルエラー |
| 2 | 処理エラー |

---

## Web版仕様

### URL

https://j1921604.github.io/Slide-Voice-Maker/

### 技術スタック

| 項目 | 技術 |
|------|------|
| UIフレームワーク | React 18（CDN） |
| JSXトランスパイル | Babel（CDN） |
| スタイリング | Tailwind CSS（CDN） |
| PDF表示 | PDF.js 3.11 |
| アイコン | Lucide（インライン） |

### 解像度設定

```javascript
const RESOLUTION_OPTIONS = [
    { label: '720p (1280x720)', value: '720p', width: 1280, height: 720 },
    { label: '1080p (1920x1080)', value: '1080p', width: 1920, height: 1080 },
    { label: '1440p (2560x1440)', value: '1440p', width: 2560, height: 1440 },
];
```

### 主要機能

1. **PDF読み込み**: PDF.jsでページを画像に変換
2. **CSV読み込み**: 複数エンコーディング対応パーサー
3. **音声生成**: Web Speech API（シミュレート）
4. **動画出力**: MediaRecorder API（WebM/VP8）
5. **GitHub Actions実行**: ワークフロー手動トリガー

---

## ファイル構造

```
Slide-Voice-Maker/
├── index.html              # GitHub Pages用Webアプリ
├── README.md               # プロジェクト説明
├── requirements.txt        # Python依存パッケージ
├── run.bat                 # ワンクリック実行（Windows）
├── run.ps1                 # PowerShell実行
├── preview.bat             # ローカルプレビュー
│
├── input/
│   ├── *.pdf               # 入力PDFファイル
│   └── 原稿.csv            # ナレーション原稿
│
├── output/
│   ├── *.webm              # 生成された動画
│   └── temp/               # 一時ファイル（自動クリア）
│
├── src/
│   ├── main.py             # メインエントリポイント
│   └── processor.py        # PDF処理・動画生成
│
├── specs/
│   └── 001-Slide-Voice-Maker/
│       ├── spec.md         # 機能仕様書
│       ├── plan.md         # 実装計画
│       ├── tasks.md        # タスク一覧
│       ├── research.md     # 調査結果
│       ├── data-model.md   # データモデル
│       ├── quickstart.md   # クイックスタート
│       └── contracts/      # API契約
│
├── docs/
│   ├── DEPLOY_GUIDE.md     # デプロイガイド
│   ├── 完全仕様書.md       # 本ファイル
│   └── ...
│
└── .github/
    └── workflows/
        ├── deploy-pages.yml    # GitHub Pagesデプロイ
        └── generate-video.yml  # 動画生成
```

---

## デプロイ

### 自動デプロイ

`main`ブランチへのpush時に自動実行。

```mermaid
flowchart LR
    A[git push main] --> B[GitHub Actions]
    B --> C[deploy-pages.yml]
    C --> D[GitHub Pages]
    D --> E[https://j1921604.github.io/Slide-Voice-Maker/]
```

### 手動設定

1. Settings → Pages → Source: **GitHub Actions**
2. Actionsタブで「Deploy to GitHub Pages」を手動実行

詳細: [DEPLOY_GUIDE.md](https://github.com/J1921604/Slide-Voice-Maker/blob/main/docs/DEPLOY_GUIDE.md)

---

## テスト

### ローカルE2Eテスト

```bash
# pytest E2E（CLI + Web）
py -3.10 -m pytest -m e2e
```

Playwrightのブラウザが未導入の場合は、事前にインストールしてください。

```bash
py -3.10 -m playwright install chromium
```

### Web版テスト

1. `index.html`をブラウザで開く
2. PDFをアップロード
3. 原稿CSVを読み込み
4. 音声生成 → WebM出力
5. ダウンロードされた動画を確認

### 成功基準

| ID | 基準 | 測定方法 |
|----|------|----------|
| SC-001 | スライド1枚あたり10秒以内 | 処理時間計測 |
| SC-002 | 選択解像度と出力解像度100%一致 | FFprobe検証 |
| SC-003 | tempフォルダが毎回クリアされる | ファイル確認 |
| SC-004 | CSV読み込みで文字化けしない | 日本語確認 |

---

## 関連リンク

- **リポジトリ**: https://github.com/J1921604/Slide-Voice-Maker
- **GitHub Pages**: https://j1921604.github.io/Slide-Voice-Maker/
- **機能仕様書**: https://github.com/J1921604/Slide-Voice-Maker/blob/main/specs/001-Slide-Voice-Maker/spec.md
- **実装計画**: https://github.com/J1921604/Slide-Voice-Maker/blob/main/specs/001-Slide-Voice-Maker/plan.md
- **タスク一覧**: https://github.com/J1921604/Slide-Voice-Maker/blob/main/specs/001-Slide-Voice-Maker/tasks.md
- **クイックスタート**: https://github.com/J1921604/Slide-Voice-Maker/blob/main/specs/001-Slide-Voice-Maker/quickstart.md
- **デプロイガイド**: https://github.com/J1921604/Slide-Voice-Maker/blob/main/docs/DEPLOY_GUIDE.md
