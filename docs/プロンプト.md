...
【ローカル実装】
下記の内容でヘルプガイド追加しろ。
- index.htmlのpdfアップロード前のindex.htmlホーム画面、pdf入力ボタンのみ配置。

- index.htmlのpdfアップロード後に展開する画面、
  原稿.csv入力ボタン実行で、毎回inputフォルダにcsvファイルを上書き保存する。ツールチップ原稿.csv入力。
  webm画像の解像度プルダウン選択する。ツールチップ画像解像度。
  画像・音声生成ボタン実行で、output\tempフォルダに音声ファイルと画像ファイルを上書き更新する。
  各スライド、音声再生、画像・音声生成する。画像・音声生成ボタンを画像・音声生成中。
  ヘッダに再生速度切り替えプルダウン。全スライドに適用しろ。
  動画に字幕ON/OFF選択する。ツールチップ字幕ON/OFF。原稿を句読点で分割し、文字数比率でタイミングを計算する。
  動画生成ボタン実行で、outputフォルダに動画webmファイルを上書き更新（pdfファイルと同名で保存）する。動画生成ボタンを動画生成中。
