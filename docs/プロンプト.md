...
【重要タスク】
index.htmlのpdfアップロード前のindex.htmlホーム画面は、pdf入力ボタンのみ配置、原稿.csv入力/出力ボタンとGithubActions実行ボタン削除しろ。
index.htmlのpdfアップロード後に展開する画面で、原稿.csv（文字化け対処）入力ボタンとGithubActions実行ボタン削除するな復元しろ。
...
- GithubActions実行ボタン
- 毎回output\temp上書き更新
- output\AIドリブン開発・教育体制の構築.webm画像の解像度を選択
- index.htmlホーム画面はpdf入力ボタンのみ配置
